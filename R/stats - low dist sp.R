# this file run regressions to predict wetland characteristics with LOW dist indicator species
# see "stats - high dist sp.R" for code to run analyses for high dist indicator species 
...
# Analyses with: Top 5 IS ####
{
  ...
  # top 5 - env vars, mostly #####
  {
    ...
    results_chars_5ter <- chars %>%
      ...
      mutate(      
        culimp_fit = map(data, ~lmer(CulImp_N ~ LowDistSp_N + (1|Protocol) + (1|Year), data=.x )),
        ...
        CanOpen_fit = map(data, ~lmer(CanopyOpenness ~ LowDistSp_N + (1|Year), data=.x )),
        ...
        OM_fit = map(data, ~lmer(OMDepth_cm ~ LowDistSp_N + (1|Year), data=.x )),
        ...
        TC_soil_fit = map(data, ~lmer(TC_soil ~ LowDistSp_N + (1|Year), data=.x )),
        ...
        pH_soil_fit = map(data, ~lmer(pH_soil ~ LowDistSp_N + (1|Year), data=.x )),
    ...
    results_chars_5wet <- chars %>%
      nest(data=-WetlandType) %>% 
      mutate(
        DO_fit = map(data, ~lmer(DO_water ~ LowDistSp_N + (1|Year), data=.x )),
        ...
        DOC_fit = map(data, ~lmer(DOC_water ~ LowDistSp_N + (1|Year), data=.x )),
        ...
        EC_fit = map(data, ~lmer(EC_water ~ LowDistSp_N + (1|Year), data=.x )),
        ...
        pH_water_fit = map(data, ~lmer(pH_water ~ LowDistSp_N + (1|Year), data=.x )),
        ...
        salinity_fit = map(data, ~lmer(Salinity_water ~ LowDistSp_N + (1|Year), data=.x )),
        ...
        TN_fit = map(data, ~lmer(TN_water ~ LowDistSp_N + (1|Year), data=.x )),
        ...
        TP_fit = map(data, ~lmer(TP_water ~ LowDistSp_N + (1|Year), data=.x )),
    ...
    sl_m <- lmer(CulImp_N ~ LowDistSp_N + (1|Year), data=filter(chars, WetlandType=="Shallow Lake") )
  ...
  # top 5 plant diversity vars ####
  {
    ...
    results_rich_top5 <- rich %>% 
      ...
      nest(data=-WetlandType) %>% 
      mutate(
        rich_fit = map(data, ~lmer(TotRichness ~ LowDistSp_N + (1|Protocol) + (1|Year), data=.x )),
        ...
        propexot_fit = map(data, ~lmer(PropExot ~ LowDistSp_N + (1|Protocol) + (1|Year), data=.x )),
    ...
    sl_rich_m <- lmer(TotRichness ~ LowDistSp_N + (1|Year), data=filter(rich, WetlandType=="Shallow Lake") )
    ...
    sl_propexot_m <- lmer(PropExot ~ LowDistSp_N + (1|Year), data=filter(rich, WetlandType=="Shallow Lake") )
...
# Analyses with: Full IS list #### 
{
  ...
  # full is list - env vars ####
  {
    ...
    # 3. perform additional lmers on shallow lake data, separate 
    sl_m <- lmer(CulImp_N ~ LowDistSp_N + (1|Year), data=filter(chars_full, WetlandType=="Shallow Lake") )
    sl_summary_stats <- tibble(WetlandType="Shallow Lake", response_var="culimp", tidy(sl_m))
    sl_fits <- tibble(WetlandType="Shallow Lake", response_var="culimp", anova(sl_m,  type=2))
    sl_aic <- tibble(WetlandType="Shallow Lake", response_var="culimp", AIC=AIC(sl_m))
  ...
  # full is list - plant diversity vars ####
  {
    ...
    # 3. now perform additional lmers on shallow lake data, separate 
    sl_rich_m <- lmer(TotRichness ~ LowDistSp_N + (1|Year), data=filter(rich_full, WetlandType=="Shallow Lake") )
    sl_rich_summary_stats <- tibble(WetlandType="Shallow Lake", response_var="rich", tidy(sl_rich_m))
    sl_rich_fits <- tibble(WetlandType="Shallow Lake", response_var="rich", anova(sl_rich_m, type=2))
    sl_rich_aic <- tibble(WetlandType="Shallow Lake", response_var="rich", AIC=AIC(sl_rich_m))
    
    sl_propexot_m <- lmer(PropExot ~ LowDistSp_N + (1|Year), data=filter(rich_full, WetlandType=="Shallow Lake") )
    sl_propexot_summary_stats <- tibble(WetlandType="Shallow Lake", response_var="propexot", tidy(sl_propexot_m))
    sl_propexot_fits <- tibble(WetlandType="Shallow Lake", response_var="propexot", anova(sl_propexot_m, type=2))
    sl_propexot_aic <- tibble(WetlandType="Shallow Lake", response_var="propexot", AIC=AIC(sl_propexot_m))
...
m_summary_full %>% 
  ...
  # View()
  filter(estimate==0)
# I'm opting to just keep Year as a random effect b/c it is a structure to the data
